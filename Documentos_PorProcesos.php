...
class docvigentes{
	...
	function __construct()
	{
		...
		$this->opcion = "Documentos Por Procesos ";
		$this->opciondetalle = "";
		$this->opcionicono = "fa fa-cog";
		$this->opcionnivel1 = "MANTENEDORES";
		$this->opcionnivel2 = "<li>Documentos Por Procesos</li>";
	...
	private function actualizar(){
		...
		//Buscamos el doccode del documento 
		if ( $this->docvigentesBD->Obtener($datos,$dt) ){
			...
			$dec5 = new dec5();
			$dec5->ObtenerDocumento($datos,$dt);
	...
	{  
		...
		if ($datos["pagina"]==1) {
			$datos["pagina_anterior"]="1"; 
			$datos["pagina_siguente"] = '1';
		}else{
			$datos["pagina_anterior"]=$datos["pagina"]-1;
		...
		$this->procesosBD->total($datos,$dt);
		...
		$this->procesosBD->todos($datos,$dt);
		$this->mensajeError .= $this->procesosBD->mensajeError;
		...
		$this->pagina->imprimirTemplate('templates/documentos_porprocesos_Listado.html');
	}

	//Mostrar listado de los registro disponibles
	private function listado_documentos()
	{
		$datos = $_REQUEST;
		...
		$this->docvigentesBD->total_pp($datos,$dt);
		$this->mensajeError.=$this->docvigentesBD->mensajeError;
		$datos["pagina_ultimo"]=$dt->data[0]["total"];
		$datos["total_registros"]=round($dt->data[0]["totalreg"]);
		...
		$this->docvigentesBD->listado_pp($datos,$dt);
		$this->mensajeError.=$this->docvigentesBD->mensajeError;
		...
		if ((isset ($datos['idCentroCosto']) && $datos['idCentroCosto'] > 0 ) || ($datos['lugarpagoid'] != '' && $datos['RutEmpresa'] != ''))
		{
			$datos['empresaid'] = $datos['RutEmpresa'];
			$this->centroscostoBD->getCombo($datos,$dt);
			$this->mensajeError = $this->centroscostoBD->mensajeError;
			$formulariox[0]["centrocosto"] = $dt->data;
		}
		...
		$formulario[0]["centrocosto"]=$formulariox[0]["centrocosto"];
		...
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/documentos_porprocesos_ListadoClasificado.html');
	}
	...
	private function detalle(){ 
		...
		$datos["opcionid"] = 'Documentos_PorProcesos.php';
		...
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/documentos_porprocesos_Detalle.html');
	}
	...
	public function verdocumento()
	{
		...
		$this->pagina->agregarDato("mensajeError",$this->mensajeError);
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/documentos_porprocesos_Documento.html');				
	}
