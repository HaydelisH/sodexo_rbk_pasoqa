				...
				if( is_numeric($key) )
				{
					...
					if(( $nombre == 'Pendiente por firma Representante') && ($estado == 2 ) )
					{
						...
						if( $datos["Firmantes_Emp"][$cont] != '' )
						{
							...
							$nuevo = array( 
								"nombre" => ucfirst(strtolower($dt3->data[0]["nombre"])).' '.ucfirst(strtolower($dt3->data[0]["appaterno"])).' '.ucfirst(strtolower($dt3->data[0]["apmaterno"])) , 
			...
			array(2) { 
				[0]=> array(2) { 
					[0]=> array(2) { 
						...
						["rut"]=> string(20) "RUT N°12181294-0" 
					} 
					[1]=> array(2) { 
						["nombre"]=> string(26) "Jerez Munoz Ricardo John " 
						["rut"]=> string(19) "RUT N°8302349-K" 
					}
				} 
				[1]=> array(1) { 
					[0]=> array(2) { 
						["nombre"]=> string(36) "Gustavo D�az Nu�es " 
						["rut"]=> string(20) "RUT N°12634720-0" 
					} 
				} 
			} 
