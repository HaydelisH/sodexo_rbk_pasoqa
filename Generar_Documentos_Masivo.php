...
$page = new generar_masivo();


class generar_masivo {
	...
	function __construct()
	{
		...
		$this->opcion = "Generar Documentos Masivo ";
		$this->opciondetalle = "";
		$this->opcionicono = "fa fa-cog";
		$this->opcionnivel1 = "MANTENEDORES";
		$this->opcionnivel2 = "<li>Generar Documentos Masivo</li>";
	...
	{	
		...
		if (isset($_REQUEST["accion2"]))
		{
			...
			switch ($_REQUEST["accion2"])
			{
				case "BUSCAR_EMP":  //AGREGAR EMPRESA
					...
					$this->generar_manualesBD->obtenerRazonSocial($datos,$dt);
					$this->mensajeError.=$this->generar_manualesBD->mensajeError;

					//Completamos los datos de la Empresa Cliente 
					$this->generar_manualesBD->obtenerRazonSocialC($datos,$dt1);
					$this->mensajeError.=$this->generar_manualesBD->mensajeError;

					//Buscamos Firmantes la Empresa
					$this->generar_manualesBD->obtenerFirmantes($datos,$dt3);
					$this->mensajeError.=$this->generar_manualesBD->mensajeError;

					//Buscamos Firmantes la Cliente
					$this->generar_manualesBD->obtenerFirmantesC($datos,$dt4);
					$this->mensajeError.=$this->generar_manualesBD->mensajeError;

					//Buscamos si la Empresa tiene Plantilla
					$this->generar_manualesBD->listadoPlantillas($datos,$dt5);
					$this->mensajeError.=$this->generar_manualesBD->mensajeError;
					...
					$this->pagina->imprimirTemplate('templates/generar_documentos_masivos_FormularioAgregar.html');
					return;
				...
				case "BUSCAR_CLI": //Buscar Cliente  
					...
					//Completamos los datos de la Empresa
					if ( $datos["RutEmpresa_Gama"] != "" ){
						$emp = array ("RutEmpresa" => $datos["RutEmpresa_Gama"]);
					}
					else{
						$emp = array ("RutEmpresa" => $datos["RutEmpresa"]);
					}
					...
					//Completamos los datos de la Empresa Cliente 
					$cli = array ("RutEmpresaC" => $datos["RutEmpresa"]); 
					...
					$this->documentosBD->obtenerPlantilla($datos,$dt6);
					...
					//Buscamos 
					if( $datos["modelo_contrato"] != 0 ){
						$this->generar_manualesBD->obtenerModeloContrato($datos,$dt4);
						$this->mensajeError.=$this->generar_manualesBD->mensajeError;
					}
					else{
						$this->pagina->agregarDato("idMC",0);
					...
					$dt->data[0]["idPla"] = $dt6->data[0]["idPlantilla"]." ".strip_tags( substr($dt6->data[0]["Descripcion_Pl"], 0, 60));
					$dt->data[0]["idPlantilla"] = $dt6->data[0]["idPlantilla"];
					$dt->data[0]["idWF"] = $datos["idWF"];
					$dt->data[0]["input_cli"] = $datos["input_cli"];
					$dt->data[0]["input_emp"] = $datos["input_emp"];
					$dt->data[0]["input_not"] = $datos["input_not"];
					$dt->data[0]["input_aval"] = $datos["input_aval"];
					
					$dt->data[0]["input_empleado"] = $datos["input_empleado"];
		...
		case "BUSCAR_P":
					...
					//Completamos los datos de la Plantilla
					$this->documentosBD->obtenerPlantilla($datos,$dt2);
					$this->mensajeError.=$this->documentosBD->mensajeError;
					
					//Consultar si el flujo tiene Notarios
					$array = array ( "idworkflow" => $dt2->data[0]["idWF"] );
					$this->flujofirmaBD->obtener_estados($array,$dt_wf);
					$this->mensajeError .= $this->flujofirmaBD->mensajeError;

					$not = $emp = $cli = $aval = $empleado = 0;
					if( count($dt_wf->data)>0 ){
						foreach ($dt_wf->data as $key => $value){
						
							if ( $dt_wf->data[$key]["idestadowf"] == 4 ){
								$not = 1;
							}
							if ( $dt_wf->data[$key]["idestadowf"] == 2 ){
								$cli = 1;
							}
							if ( $dt_wf->data[$key]["idestadowf"] == 3 ){
								$emp = 1;
							}
							if ( $dt_wf->data[$key]["idestadowf"] == 5 ){
								$aval = 1;
							}
							if ( $dt_wf->data[$key]["idestadowf"] == 7 ){
								$empleado = 1;
							}
						}
					}
					...
					$dt->data[0]["input_not"] = $not;
					$dt->data[0]["input_aval"] = $aval;
					$dt->data[0]["input_emp"] = $emp;
					$dt->data[0]["input_cli"] = $cli;
					$dt->data[0]["input_empleado"] = $empleado;
		...
		case "BUSCAR_N": 
					...
					$this->generar_manualesBD->obtenerRazonSocialN($datos,$dt2);
					$this->mensajeError.=$this->generar_manualesBD->mensajeError;
					...
					$this->generar_manualesBD->obtenerFirmantesN($datos,$dt5);
					$this->mensajeError.=$this->generar_manualesBD->mensajeError;
					...
					//Consultar si el flujo tiene Notarios
					$array = array ( "idworkflow" => $dt6->data[0]["idWF"] );
					$this->flujofirmaBD->obtener_estados($array,$dt_wf);
					$this->mensajeError .= $this->flujofirmaBD->mensajeError;
					...
					$this->generar_manualesBD->obtenerTipoFirma($datos,$dt9);
					$this->mensajeError.=$this->generar_manualesBD->mensajeError;
					...
					$dt->data[0]["RutEmpresaN"] = $dt2->data[0]["RutEmpresaN"];
					$dt->data[0]["idPla"] = $dt6->data[0]["idPlantilla"]." ".strip_tags( substr($dt6->data[0]["Descripcion_Pl"], 0, 60));
					$dt->data[0]["idWF"] = $dt6->data[0]["idWF"];
					$dt->data[0]["idPlantilla"] = $dt6->data[0]["idPlantilla"];
					$dt->data[0]["NombreWF"] = $dt6->data[0]["nombrewf"];
					$dt->data[0]["idTipoFirma"] = $dt9->data[0]["idTipoFirma"];
					$dt->data[0]["Descripcion"] = $dt9->data[0]["Descripcion"];
					$dt->data[0]["input_not"] = $datos["input_not"];
					$dt->data[0]["input_aval"] = $datos["input_aval"];
	...
	{  
        ...
        include_once("importarExcel_Masivo.php");
	...
	private function buscar_empresa(){
		...
		$this->pagina->imprimirTemplate('templates/generar_documentos_masivos_FormularioAgregarEmpresa.html');
	}
	...
	private function buscar_cliente(){
		...
		//Buscar todas las Empresas disponibles
		$this->generar_manualesBD->listadoClientesDiferente($datos,$dt);
		$this->mensajeError.=$this->generar_manualesBD->mensajeError;

		if( count($dt->data) > 0){
			foreach ($dt->data as $key => $value) {
				$dt->data[$key]["RutEmpresa_Gama"] = $datos["RutEmpresa"];
				$dt->data[$key]["idPlantilla"] = $datos["idPlantilla"];
				$dt->data[$key]["idWF"] = $datos["idWF"];
				$dt->data[$key]["input_not"] = $datos["input_not"];
				$dt->data[$key]["input_aval"] = $datos["input_aval"];
				$dt->data[$key]["input_emp"] = $datos["input_emp"];
				$dt->data[$key]["input_cli"] = $datos["input_cli"];
				$dt->data[$key]["input_empleado"] = $datos["input_empleado"];
			}
		}
		...
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/generar_documentos_masivos_FormularioAgregarCliente.html');
	}
	...
	private function buscar_plantilla(){
		...
		//Listado de Empresas Disponibles 
		$this->documentosBD->listadoPlantillas($datos,$dt);
		$this->mensajeError.=$this->documentosBD->mensajeError;
		
		if( count($dt->data) > 0){
			foreach ($dt->data as $key => $value) {
				$dt->data[$key]["RutEmpresa"] = $datos["RutEmpresa"];
				$dt->data[$key]["RutEmpresaC"] = $datos["RutEmpresaC"];
				$dt->data[$key]["TipoFirmas"] = $datos["TipoFirmas"];
				$dt->data[$key]["input_not"] = $datos["input_not"];
				$dt->data[$key]["input_aval"] = $datos["input_aval"];
				$dt->data[$key]["input_emp"] = $datos["input_emp"];
				$dt->data[$key]["input_cli"] = $datos["input_cli"];
				$dt->data[$key]["input_empleado"] = $datos["input_empleado"];
				$dt->data[$key]["modelo_contrato"] = $datos["modelo_contrato"];
		...
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/generar_documentos_masivos_FormularioAgregarPlantillas.html');
	}
	...
	private function buscar_notaria(){
		...
		//Buscar todas las Empresas disponibles
		$this->generar_manualesBD->listadoEmpresas($datos,$dt);
		$this->mensajeError.=$this->generar_manualesBD->mensajeError;
		...
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/generar_documentos_masivos_FormularioAgregarNotaria.html');
	}
	...
	private function verdocumento($data){
		...
		$this->generar_manualesBD->obtenerb64($datos,$dt); 		
		$this->mensajeError=$this->generar_manualesBD->mensajeError;
		...
		$this->pagina->agregarDato("mensajeError",$this->mensajeError);
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/generar_documentos_masivos_documento.html');					
	}
