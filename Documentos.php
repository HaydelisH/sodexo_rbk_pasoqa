...
class documentos {
	...
	private $orientacion = 'portrait';//Vertical
	...
	//Seleccionar los Contratos Marco de la Empresa Cliente  
	private function buscar_cm(){
		...
		$this->pagina->agregarDato("mensajeError",$this->mensajeError);
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/documentos_FormularioAgregarCM.html');
	}
		...
		if( count($dt->data) > 0 ){
			foreach ($dt->data as $key => $value) {
				...
				if ( count($dt2->data) > 0 ){
					...
					$dt->data[$key]["id_detalle"] = 1;
				}
				else{
					$dt->data[$key]["detalle"] = $this->amarillo1;
					$dt->data[$key]["id_detalle"] = 0;
				}
			}
		}
	...
	private function firmantes(){
		...
		if( $datos["modelo_contrato"] == 1 ) {//Si es Contrato Marco
			...
			$this->documentosBD->idMaxContratos($dtmax);
			$this->mensajeError = $this->documentosBD->mensajeError;
			$aux = $dtmax->data[0]["idDocumento_Gama"];

			//Separar el id de Documento
			$aux_2 = explode("_", $aux); 
			$num = intval($aux_2[1]);
			$num++;
			$num_nuevo = $aux_2[0]."_".$num;

			//Pasar al formulario
			$dt->data[0]["idDocumento"] = $num_nuevo;
		}
		...
		/*Proveedores*/
		$this->agregarProveedores($this->proveedores);	
		$variable =  count($this->proveedores);
			...
			switch ($variable) {
				case 1:
					for ($i=0; $i < 3; $i++) { 
						...
						if( $this->proveedores[0]["rut_p_".$i] != "" ){
							$dt->data[0]["rut_p_0"] = $this->proveedores[0]["rut_p_".$i];
							$dt->data[0]["nombre_p_0"] = $this->proveedores[0]["nombre_p_".$i];
							$dt->data[0]["detalle_p_0"] = $this->proveedores[0]["detalle_p_".$i];
							$dt->data[0]["monto_p_0"] = $this->proveedores[0]["monto_p_".$i];
						}
					}
				...
				case 2:
					foreach ($this->proveedores as $key => $value) {
						...
						if ( $this->proveedores[$key]["rut_p_0"] != "" ){

							$dt->data[0]["rut_p_0"] = $this->proveedores[$key]["rut_p_0"];
							$dt->data[0]["nombre_p_0"] = $this->proveedores[$key]["nombre_p_0"];
							$dt->data[0]["detalle_p_0"] = $this->proveedores[$key]["detalle_p_0"];
							$dt->data[0]["monto_p_0"] = $this->proveedores[$key]["monto_p_0"];
						}else{

							$dt->data[0]["rut_p_0"] = $this->proveedores[$key]["rut_p_1"];
							$dt->data[0]["nombre_p_0"] = $this->proveedores[$key]["nombre_p_1"];
							$dt->data[0]["detalle_p_0"] = $this->proveedores[$key]["detalle_p_1"];
							$dt->data[0]["monto_p_0"] = $this->proveedores[$key]["monto_p_1"];
						}
						if ( $this->proveedores[$key]["rut_p_1"] != "" ){

							$dt->data[0]["rut_p_1"] = $this->proveedores[$key]["rut_p_1"];
							$dt->data[0]["nombre_p_1"] = $this->proveedores[$key]["nombre_p_1"];
							$dt->data[0]["detalle_p_1"] = $this->proveedores[$key]["detalle_p_1"];
							$dt->data[0]["monto_p_1"] = $this->proveedores[$key]["monto_p_1"];
						}else{

							$dt->data[0]["rut_p_1"] = $this->proveedores[$key]["rut_p_2"];
							$dt->data[0]["nombre_p_1"] = $this->proveedores[$key]["nombre_p_2"];
							$dt->data[0]["detalle_p_1"] = $this->proveedores[$key]["detalle_p_2"];
							$dt->data[0]["monto_p_1"] = $this->proveedores[$key]["monto_p_2"];
						}
				...
				case 3:
					$dt->data[0]["rut_p_0"] = $this->proveedores[0]["rut_p_0"];
					$dt->data[0]["nombre_p_0"] = $this->proveedores[0]["nombre_p_0"];
					$dt->data[0]["detalle_p_0"] = $this->proveedores[0]["detalle_p_0"];
					$dt->data[0]["monto_p_0"] = $this->proveedores[0]["monto_p_0"];
				
					$dt->data[0]["rut_p_1"] = $this->proveedores[1]["rut_p_1"];
					$dt->data[0]["nombre_p_1"] = $this->proveedores[1]["nombre_p_1"];
					$dt->data[0]["detalle_p_1"] = $this->proveedores[1]["detalle_p_1"];
					$dt->data[0]["monto_p_1"] = $this->proveedores[1]["monto_p_1"];
				
					$dt->data[0]["rut_p_2"] = $this->proveedores[2]["rut_p_2"];
					$dt->data[0]["nombre_p_2"] = $this->proveedores[2]["nombre_p_2"];
					$dt->data[0]["detalle_p_2"] = $this->proveedores[2]["detalle_p_2"];
					$dt->data[0]["monto_p_2"] = $this->proveedores[2]["monto_p_2"];
					break;
			}
			
		}

		//Verificar que flujo trae la plantilla 
		$flujo_c = 0;
		$flujo_e = 0;
		$flujo_n = 0;
		...
		if ( count($dt10->data) > 0 ){
			...
			foreach ($dt10->data as $key => $value) {
				
				switch ($dt10->data[$key]["idEstado"]){
					case 2 : $flujo_c = 1; break;
					case 3 : $flujo_e = 1; break;
					case 4 : $flujo_n = 1; break;
				}
			}
		}

		//Asignacion de datos 
		$dt->data[0]["cli"] = $flujo_c;
		$dt->data[0]["emp"] = $flujo_e;
		$dt->data[0]["not"] = $flujo_n;
	...
	private function generar(){
		...
		if( $datos["idDocumento"] == '' ){
			$datos["idDocumento"] = $datos["idDocumento_Gama"];
		}
		if( $datos["idDocumento_Gama"] == '' ){
			$datos["idDocumento_Gama"] = $datos["idDocumento"];
		}
		...
		switch ($datos["modelo_contrato"]) {
			case 1: //Contrato Marco 
				...
				break;
			case 2: //Anexo
					$this->orientacion = 'Landscape';// Horizontal  
		...
		$resultado .= $personerias."</div>";

		//Si es de firma manual
		if( $datos["TipoFirmas"] == 1 ){
			$this->construirTablaFirmantes($idContrato,$this->firmantes_completos, $this->firmantes_tabla);
			//Unir texto y tabla de Firmantes	
		    $resultado .= $this->firmantes_tabla;
		}

		$resultado = "<html><body>".$resultado."</body></html>";

		$this->GrabaLog($resultado);//HTML Original
		//$this->GrabaLog($this->orientacion);//Orientacion del papel
		//$this->GrabaLog($datos["modelo_contrato"]);//modelo de contrato
		
		//Codificacion internacional del texto
    	$texto_completo = utf8_encode($resultado);

    	//Sustituir acentos 
		$texto_completo = $this->TildesHtml($texto_completo);
...
p { orphans: 3; widows: 1}</style>
        ...
        if( count($dt->data) > 0){
        	...
        	foreach ($dt->data as $i => $value) {
	        	...
	        	if( $dt->data[$i]["Encabezado"] == 1){
	        		...
	        		$dt->data[$i]["Descripcion_Cl"] = "<p><strong><u>".$resultado."</u></strong>: ";
	        	...
	        	if( $dt->data[$i]["Titulo"] == 0 ){
	        		...
	        		$html .= "<p><strong>".$dt->data[$i]["Titulo_Cl"]."</strong><br>";
	        	}
	        	...
	        	if( substr($dt->data[$i]["Texto"], 0 ,30 ) == '<p style="text-align:justify">'){
	        		$cla = substr($dt->data[$i]["Texto"],30);
	        	}else{
	        		$cla = $dt->data[$i]["Texto"];
	        	}
		
	        	$aux = strip_tags($cla,'<ul><li><p><ol><strong><table><tr><td><th><tbody><tfoot>');
				
				//Sustituir etiquetas
				$aux = str_replace('</p>','<br>',$aux);
				$aux = str_replace('<p style="text-align:justify"><strong>', '<strong>', $aux);
	...
	private function construirPersonerias(&$resultado){
		...
		$encabezado = "<p><strong>PERSONER&Iacute;AS</strong>";
		...
		if( count($firmantes_emp) > 0 ){
			...
			foreach ($firmantes_emp as $key => $value) {
				...
				if ( $firmantes_emp_per[$key] == 1 ){
					...
					if( $datos != "" ){
						$num++; 
						$texto .=  "<br><br>".$num.") ".strip_tags($datos);
					}
				}
			}
		}
	...
	public function sustituirVariables($datos, $html,&$resultado){
       ...
       //Buscar datos del Representante 6
		...
		//Graba Log Variables
		$this->GrabaLog_Variables("Function SustituirVariables( Contrato Marco y Condiciones especiales ) -> Arreglo de Variables ");
	...
	public function sustituirVariablesAnexo($datos, $html,&$resultado){
		...
		//Graba Log Variables
		$this->GrabaLog_Variables("Function SustituirVariablesAnexo( Contrato Anexo ) -> Arreglo de Variables ");
		foreach ($variables as $clave => $valor) {
			$this->GrabaLog_Variables($clave." = ".$valor);
		}
	...
	public function sustituirVariables_2($datos, $html,&$resultado){
       ...
       //Buscar datos del Representante 6
		...
		//Graba Log Variables
		$this->GrabaLog_Variables("Function SustituirVariables_2( Contrato Renting ) -> Arreglo de Variables ");
		foreach ($variables as $clave => $valor) {
			$this->GrabaLog_Variables($clave." = ".$valor);
		}
		//Guardar en la BD
	...
	public function sustituirVariables_3($datos, $html,&$resultado){
       ...
       //Buscar datos del Representante 6
		...
		//Graba Log Variables
		$this->GrabaLog_Variables("Function SustituirVariables_3( Contrato Financiero/Operativo ) -> Arreglo de Variables ");
		foreach ($variables as $clave => $valor) {
			$this->GrabaLog_Variables($clave." = ".$valor);
		}
	...
	public function agregarProveedores(&$resultado){
		...
		if ( count($aux_1) > 0 ){
			array_push($completo, $aux_1);
		}

		if ( count($aux_2) > 0 ){
			array_push($completo, $aux_2);
		}

		if ( count($aux_3) > 0 ){
			array_push($completo, $aux_3);
		}
	...
	//Grabar log de las variables del Documento
	private function GrabaLog_Variables($datos){
		//graba log del xml
		date_default_timezone_set('America/Santiago');
		$time = time();
		$nomarchivo = 'logs/logVariables'.@date("Ymd").'.TXT';	
		$ar=fopen($nomarchivo,"a") or
		   die("Problemas en la creacion");
		fputs($ar,@date("H:i:s",$time)." ".$datos);
		fputs($ar,"\n");
	...
	//Grabar log de datos de Proveedores
	private function GrabaLog_Proveedores($datos){
		//graba log del xml
		date_default_timezone_set('America/Santiago');
		$time = time();
		$nomarchivo = 'logs/logProveedores'.@date("Ymd").'.TXT';	
		$ar=fopen($nomarchivo,"a") or
		   die("Problemas en la creacion");
		fputs($ar,@date("H:i:s",$time)." ".$datos);
		fputs($ar,"\n");
