...
class importar
{
    ...
    private $path = 'tmp/';
	private $nombreNemotecnicoArchivo = 'resultadoPostulacion_';
    ...
    {
        //var_dump($data);// array(4) { ["rut"]=> string(10) "14137454-0" ["estadoPostulacion"]=> string(1) "2" ["RutEmpresa"]=> string(10) "76012676-4" ["idCargoEmpleado"]=> string(3) "123" }
        $this->postulacionBD->existePostulacion2($data, $dt);
        ...
        else
        {
            if (count($dt->data) > 0)
            {
                ...
                $this->postulacionBD->resultadoPostulacionUpdate($data);
                $this->mensajeError.=$this->postulacionBD->mensajeError;
                if ($this->mensajeError != '')
                {
                    $respuesta['estado'] = false;
                ...
                else
                {
                    ...
                    $respuesta['mensaje'] = 'Se actualizo la postulacion para el R.U.T. ' . $data['rut'];
                }
            }
            else
            {
