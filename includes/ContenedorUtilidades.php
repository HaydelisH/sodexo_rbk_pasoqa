...
class ContenedorUtilidades
{
	...
	public static function TildesHtml($cadena){ 
        ...
        return str_replace(array("á","é","í","ó","ú","ñ","Á","É","Í","Ó","Ú","Ñ","°","º","> <"," ","<span> ","¿","?"),
						   array("&aacute;","&eacute;","&iacute;","&oacute;","&uacute;","&ntilde;",
								"&Aacute;","&Eacute;","&Iacute;","&Oacute;","&Uacute;","&Ntilde;","&deg;","&deg;",">&nbsp;<","","<span>&nbsp;","&iquest;","&#63;"), $cadena);     
    }
	...
	public static function checkClaveRobusta($clave, $tipo,$minimos)
	{
		switch($tipo)
		{
			...
			case 'robustes1':// La clave debe contener numeros
				$respuesta = ContenedorUtilidades::checkNumero($clave,$minimos['minNumber']);
			break;
			case 'robustes2':// La clave debe contener letras mayusculas y minusculas
				$chk1 = ContenedorUtilidades::checkMinusculas($clave,$minimos['minMinus']);
				$chk2 = ContenedorUtilidades::checkMayusculas($clave,$minimos['minMayus']);
			...
			case 'robustes3':// La clave debe contener numeros y letras mayusculas y minusculas
				$chk1 = ContenedorUtilidades::checkMinusculas($clave,$minimos['minMinus']);
				$chk2 = ContenedorUtilidades::checkMayusculas($clave,$minimos['minMayus']);
				$chk3 = ContenedorUtilidades::checkNumero($clave,$minimos['minNumber']);
			...
			break;			
			case 'robustes4':// La clave debe contener numeros y letras mayusculas y minusculas y un caracter especial
				$chk1 = ContenedorUtilidades::checkMinusculas($clave,$minimos['minMayus']);
				$chk2 = ContenedorUtilidades::checkMayusculas($clave,$minimos['minMinus']);
				$chk3 = ContenedorUtilidades::checkNumero($clave,$minimos['minNumber']);
				$chk4 = ContenedorUtilidades::checkMinimoCaracterEspecial($clave,$minimos['minCaracterEspecial']);
				if ($chk1['exito'] && $chk2['exito'] && $chk3['exito'] && $chk4['exito'])
				{
					$respuesta = array('exito'=>true);
				}
				else
				{
					$respuesta = array(
						'exito'=>false, 
						'mensaje'=>(isset($chk1['mensaje']) ? $chk1['mensaje'] : '' ) . (isset($chk2['mensaje']) ? $chk2['mensaje'] : '') . (isset($chk3['mensaje']) ? $chk3['mensaje'] : '') . (isset($chk4['mensaje']) ? $chk4['mensaje'] : ''));
				}
			break;
		}
		return $respuesta;
	...
	public static function checkMinusculas($clave, $cantidadMinima = 1)
    {
        // Verificar la cantidad de minusculas en la clave
		preg_match_all('/[a-z]/', $clave, $matches);
		$cantidadMinus = count($matches[0]);
		// Verificar si la cantidad es al menos la requerida
		if ($cantidadMinus < $cantidadMinima) {
			$mensaje = sprintf(
				'<p>La clave debe tener al menos %d letra(s) minuscula(s).</p>',
				$cantidadMinima,
				$cantidadMinima > 1 ? 'es' : '',
				$cantidadMinima > 1 ? 'es' : ''
			);
	...
	public static function checkMayusculas($clave, $cantidadMinima = 1)
    {
		// Verificar la cantidad de mayusculas en la clave
		preg_match_all('/[A-Z]/', $clave, $matches);
		$cantidadMayus = count($matches[0]);
		// Verificar si la cantidad es al menos la requerida
		if ($cantidadMayus < $cantidadMinima) {
			$mensaje = sprintf(
				'<p>La clave debe tener al menos %d letra(s) may&uacute;scula(s).</p>',
				$cantidadMinima,
				$cantidadMinima > 1 ? 'es' : '',
				$cantidadMinima > 1 ? 'es' : ''
			);
	...
	public static function checkNumero($clave, $cantidadMinima = 1)
    {
        // Verificar la cantidad de numeros en la clave
		preg_match_all('/[0-9]/', $clave, $matches);
		$cantidadNum = count($matches[0]);
		
		// Verificar si la cantidad es al menos la requerida
		if ($cantidadNum < $cantidadMinima) {
			$mensaje = sprintf(
				'<p>La clave debe tener al menos %d caracter(es) num&eacute;rico(s).</p>',
				$cantidadMinima,
				$cantidadMinima > 1 ? 'es' : '',
				$cantidadMinima > 1 ? 'es' : ''
			);
	...
	public static function checkMinimoCaracterEspecial($clave, $cantidadMinima = 1)
	{
		// Verificar la cantidad de caracteres especiales en la clave
		preg_match_all('/[!@#$%^&*()_+{}\[\]:;<>,.?~\\/-]/', $clave, $matches);
		$cantidadCaracteresEspeciales = count($matches[0]);

		// Verificar si la cantidad es al menos la requerida
		if ($cantidadCaracteresEspeciales < $cantidadMinima) {
			$mensaje = sprintf(
				'<p>La clave debe tener al menos %d caracter%s especial%s.</p>',
				$cantidadMinima,
				$cantidadMinima > 1 ? 'es' : '',
				$cantidadMinima > 1 ? 'es' : ''
			);
