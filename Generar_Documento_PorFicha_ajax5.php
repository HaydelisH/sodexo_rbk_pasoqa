...
class vistaprevia {
	...
	function __construct()
	{
		...
		$var = new variables();
		$var->construirVariablesValoresSinDocumento($datos,VAR_DOCUMENTO,$resultado_documento); 
		...
		if( count($resultado_documento) > 0 ){
			$resultado['variables'] = array_merge($resultado['variables'],$resultado_documento['variables']);
			$resultado['valores'] = array_merge($resultado['valores'],$resultado_documento['valores']);
		}
		...
		if( $datos["idTipoFirma"] == 1 ){
					
					$firmantes_tabla = '';
					$firmantes_completos = array();
					$gen->crearFirmantesSinDocumento($datos,$firmantes_completos);
					$gen->construirTablaFirmantesSinDocumento($firmantes_completos, $firmantes_tabla);
					
					//Unir texto y tabla de Firmantes	
					$html .= $firmantes_tabla;
				}

				$html .= "</body></html>";
		echo utf8_encode(ContenedorUtilidades::TildesHtml($html));
	}
