...
class firma {
	...
	public function CargarDocumento($datos,&$resultadotrx)
	{
		...
		//var_dump ($datos["firmantesdatos"]);
		$cantidadfirmantes = count($datos["firmantesdatos"]);
		for ($f=0; $f < $cantidadfirmantes; $f++) 
		{
			
			$rut_con_guion = '';
			$this->colocarGuiondelRut($datos["firmantesdatos"][$f]["firmanteid"],$rut_con_guion);
			
			$datos["firmanteid"] 	= $rut_con_guion;
			$this->firmantesBD->obtenerXusuario($datos,$dtFirm);
			
			$arr9["countryCode"] 				= $this->countryCode;
			
			
			$arr9["personalNumber"] 			= $datos["firmantesdatos"][$f]["firmanteid"];
			if (count($dtFirm->data)== 0 && $datos["firmantesdatos"][$f]["firmanterol"] == "REPR")
			{
				$arr9["personalNumber"]         = $datos["firmantesdatos"][$f]["firmanteid"];
			}	
	...
	private function ejecutarFirma($datos){	
		...
		if( $this->mensajeError != '' ){
			...
			else if (strpos($this->mensajeError, '500') && strpos($this->mensajeError, 'Firmante no posee ROL') ) {
				$this->mensajeError = "El firmante no posee ROL requerido para firmar el documento";
				return false;
			}
		...
		$this->valor_arr = "";
		//ini para obtener fecha firma de api
		//$this->obtener_dato_arr($dt,"timestamp");
		$this->obtener_dato_arr($dt,"createdAt");
		//fin
	...
	private function colocarGuiondelRut($rut, &$rut_con_guion){

		$largo = strlen($rut);
		$resultado[0] = substr($rut,0,$largo - 1);
		$resultado[1] = substr($rut,$largo -1 , 1);
		$rut_con_guion = $resultado[0].'-'.$resultado[1];
		return $rut_con_guion;
	}
