...
class docvigentes{
	...
	{   
		...
		$this->docvigentesBD->rl_totalPorEstructura($datos,$dt);
		$this->mensajeError.=$this->docvigentesBD->mensajeError;
		...
		$this->docvigentesBD->rl_listadoPorEstructura($datos,$dt);
		$this->mensajeError.=$this->docvigentesBD->mensajeError;
		...
		for ($l=0; $l < count($dt->data) ; $l++)
		{
			...
			if( isset($datos['lugarpagoid'])) $formulariox[0]["listado"][$l]['lugarpagoid'] = $datos['lugarpagoid'];
			if( isset($datos['idCentroCosto'])) $formulariox[0]["listado"][$l]['idCentroCosto'] = $datos['idCentroCosto'];
		}
		...
		$this->rl_tiposdocumentosBD->Todos($dt);
		...
		//$this->mensajeError.=$this->firmasdocBD->mensajeError;
		$this->estadosworkflowBD->rl_listado($dt3);
		...
		$this->rl_proveedoresBD->listadoCombo($dt);
		$this->mensajeError.=$this->rl_proveedoresBD->mensajeError;
		$formulariox[0]["Institucion"] = $dt->data;
		...
		/*if( (isset ($datos['lugarpagoid']) && $datos['lugarpagoid'] > 0 ) || $datos['RutEmpresa'] != '')
		{
		...
		$formulario[0]["Institucion"]=$formulariox[0]["Institucion"];	
		...
		$this->pagina->imprimirTemplate('templates/rl_documentos_vigentes_Listado.html');
	}
	...
	private function detalle(){ 
		...
		$datos["opcionid"] = 'rl_Documentos_Vigentes.php';
		...
		//Buscar permisos por perfil de usuario
		$this->tiposusuariosBD->obtener($datos,$dt);
		$this->mensajeError.=$this->tiposusuariosBD->mensajeError;
		if( $dt->data[0]['renotificar'] ){
			$formulario[0]["renotificar"][0] = "";
			for ($i = 0; $i < count($formulario[0]["contratofirmantes"]); $i++){
				$dat = array(
					'idDocumento'=>$datos['idDocumento'],
					'RutUsuario'=>$formulario[0]["contratofirmantes"][$i]['RutFirmante']
				);
				$this->enviocorreosBD->puedeRenotificar($dat,$dtt);
				$this->mensajeError.=$this->enviocorreosBD->mensajeError;
				if ($dtt->data[0]['FechaCreacion']){
					$formulario[0]["contratofirmantes"][$i]["renotificar"][0]['RutFirmante'] = $formulario[0]["contratofirmantes"][$i]['RutFirmante'];
				}
			}
		}
		...
		if( isset($datos['RutEmpresa'])) $formulario[0]['RutEmpresa'] = $datos['RutEmpresa'];
		if( isset($datos['lugarpagoid'])) $formulario[0]['lugarpagoid'] = $datos['lugarpagoid'];
		if( isset($datos['idCentroCosto'])) $formulario[0]['idCentroCosto'] = $datos['idCentroCosto'];
		...
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/rl_documentos_vigentes_Detalle.html');
	}
	...
	public function verdocumento()
	{
		...
		$this->pagina->agregarDato("mensajeError",$this->mensajeError);
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/rl_documentos_vigentes_Documento.html');				
	}
