...
class firmasdoc {
	...
	{  	
		...
		$this->docvigentesBD->rl_totalMisDocumentos($datos,$dt);
		...
		$this->docvigentesBD->rl_listadoMisDocumentos($datos,$dt);
		$this->mensajeError.=$this->docvigentesBD->mensajeError;
		...
		$this->pagina->imprimirTemplate('templates/rl_documentos_pendientes_Listado.html');
	...
	{  
 	...
 	private function detalle(){// creamos una nueva instancia de la tabla
		...
		$datos["opcionid"] = 'rl_Documentos_Pendientes.php';
		...
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/rl_documentos_pendientes_Detalle.html');
	...
	private function verdocumento()
	{
		...
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/rl_documentos_pendientes_documento.html');				
	}
	...
	private function InicioFirma_RBK()
	{
		...
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/rl_documentos_Pendientes_firma_rbk.html');	
	}
	...
	private function InicioFirmaPin()
	{
		...
		$this->pagina->agregarDato("mensajeError",$this->mensajeError);
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/rl_documentos_pendientes_firma.html');			
	}
	...
	private function InicioFirmaToken()
	{
		...
		$this->pagina->agregarDato("mensajeOK",$this->mensajeOK);
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/rl_documentos_pendientes_firma_token.html');	
	...
	private function InicioFirmaHuella()
	{
		...
		$this->pagina->agregarDato("mensajeError",$this->mensajeError);
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/rl_documentos_pendientes_firma_huella.html');	
		$this->imprimirFin2();//se crea pie de pagina dos, por incompatibilidad con los script para multibrowser
		exit;//para finalizar ya que se imprimió el pie de pagina en linea anterior		
	...
	private function firmar(){
		...
		switch (GESTOR_FIRMA) {
			...
			default:
				$this->documentosBD->esRelacionLaboral_obtener($datos, $dt);
				$this->mensajeError.=$this->documentosBD->mensajeError;
				if ($dt->data[0]['tipoWF'] == 1)
				{
					//var_dump("123456790987654323456789098765");
