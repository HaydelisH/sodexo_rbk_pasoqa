...
class firmasdoc {
	...
	function __construct()
	{
		...
		$this->opcion = "Mis Documento";
		$this->opciondetalle = "";
		$this->opcionicono = "fa fa-cog";
		$this->opcionnivel1 = "MANTENEDORES";
		$this->opcionnivel2 = "<li>Mis Documentos</li>";
	...
	{  	
		...
		// pedimos el listado
		$datos=$_REQUEST;
		if( ! isset($datos['idEstado']) ) //|| $datos['idEstado'] == 0 ) // Este caso se comenta pues cuando se navega desde el dashboard(Documentos en Proceso de Firma) idEstado vale 0 y se visualizan cosas que no se deben ver segun origen de navegacion, en este caso, desde el dashboard
			$datos["idEstado"] = -1; 
		...
		$this->docvigentesBD->total_misdocumentos($datos,$dt);
		...
		//busco el total de paginas
		$this->docvigentesBD->listado_misdocumentos($datos,$dt);
		$this->mensajeError.=$this->docvigentesBD->mensajeError;
		...
		$formulariox[0]["plantillas"]=$dt->data;
		
		//Listado de estados
		//$this->firmasdocBD->listadoPorEstados($datos,$dt3);
		//$this->mensajeError.=$this->firmasdocBD->mensajeError;
		//$this->estadocontratosBD->listado($dt3);
		$this->estadocontratosBD->listado_misdocumentos($dt3);
		...
		$formulario[0]["plantillas"]=$formulariox[0]["plantillas"];
		...
		$this->pagina->agregarDato("autoFiltro", ContenedorUtilidades::autoFiltro('misDocumentos', $datos)['datos2Filtro']);
		// Filtro de busqueda y navegacion

		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/misdocumentos_Listado.html');
	...
	{  
 	...
 	private function detalle(){// creamos una nueva instancia de la tabla
		...
		$datos["opcionid"] = 'MisDocumentos.php';
		...
		// Filtro de busqueda y navegacion
		$this->pagina->agregarDato("autoFiltroRetorno", ContenedorUtilidades::autoFiltro('misDocumentos', $datos)['filtro2datos']);
		$this->pagina->agregarDato("autoFiltro", ContenedorUtilidades::autoFiltro('misDocumentos', $datos)['filtro2Filtro']);
		// Filtro de busqueda y navegacion

		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/misdocumentos_Detalle.html');
	...
	private function verdocumento()
	{
		...
		// Filtro de busqueda y navegacion
		$this->pagina->agregarDato("autoFiltroRetorno", ContenedorUtilidades::autoFiltro('misDocumentos', $datos)['filtro2Filtro']);
		// Filtro de busqueda y navegacion
		...
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/misdocumentos_documento.html');				
	}
	...
	{ 
		...
		// Filtro de busqueda y navegacion
		ContenedorUtilidades::autoFiltro('misDocumentos', $datos, $_REQUEST);
		// Filtro de busqueda y navegacion
	...
	private function InicioFirma_RBK()
	{
		...
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/misdocumentos_firma_rbk.html');	
	}
	...
	private function InicioFirmaPin()
	{
		...
		$this->pagina->agregarDato("mensajeError",$this->mensajeError);
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/misdocumentos_firma.html');			
	}
	...
	private function InicioFirmaToken()
	{
		...
		$this->pagina->agregarDato("mensajeOK",$this->mensajeOK);
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/misdocumentos_firma_token.html');	
	...
	private function InicioFirmaHuella()
	{
		...
		$this->pagina->agregarDato("mensajeError",$this->mensajeError);
		// se imprime el template con la pagina
		$this->pagina->imprimirTemplate('templates/misdocumentos_firma_huella.html');	
		$this->imprimirFin2();//se crea pie de pagina dos, por incompatibilidad con los script para multibrowser
		exit;//para finalizar ya que se imprimió el pie de pagina en linea anterior		
	...
	public function por_firma(){
		...
		//$this->firmasdocBD->listadoPorTiposDocumentos($datos,$dt);
		//$this->mensajeError.=$this->firmasdocBD->mensajeError;
		$this->tiposdocumentosBD->Todos($dt_1);
		$this->mensajeError .= $this->tiposdocumentosBD->mensajeError;
		$formulariox[0]["tiposdocumentos"]=$dt_1->data;
