...
class Cargos {
	...
	{	
		...
		$this->pagina->imprimirTemplate('templates/cargos_FormularioAgregar.html');
	}
	...
	{	
		...
		if (isset($datos["accion2"]))
		{
			switch ($datos["accion2"])
			{
				case "MODIFICAR":
					...
					if ($this->cargosBD->modificar($datos,$dt))
					{
		...
		$this->pagina->imprimirTemplate('templates/cargos_FormularioModificar.html');
	}
	...
	{	
		...
		if ($this->cargosBD->eliminar($datos, $dt)){
	...
	{  
		...
		$this->pagina->imprimirTemplate('templates/cargos_Listado.html');
	}
