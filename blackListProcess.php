...
class importar
{
	...
	private function llamadaCurl($datos)
	{
		...
		$url= (isset($dt2->data[0]['parametro']) ? $dt2->data[0]['parametro'] : "http://localhost/SMU_RBK") . "/blackListProcess.php";
		$this->graba_log_curl("url:".$url);
		$parametros = "fila={$datos['fila']}&usuarioid={$datos['usuarioid']}&IdArchivo={$datos['IdArchivo']}&to={$datos['to']}&accion=LOOP";
    ...
    {
        //var_dump($data);// array(1) { ["rut"]=> string(10) "11457163-6" }
        $this->postulacionBD->listaNegraUpdate($data);
        ...
        {  
            ...
            if ($data['to'] == 1)
            {
                $respuesta['mensaje'] = 'No se pudieron aplicar las politicas de empresa al R.U.T. ' . $data['rut'];
            }
            else
            {
                $respuesta['mensaje'] = 'No se pudo retirar el R.U.T. ' . $data['rut'];
            }
        }
        else
        {
            ...
            if ($data['to'] == 1)
            {
                $respuesta['mensaje'] = 'El R.U.T. ' . $data['rut'] . ' se agrego';
            }
            else
            {
                $respuesta['mensaje'] = 'El R.U.T. ' . $data['rut'] . ' se retiro';
            }
        }
        return $respuesta;
    }
